...
<div id="main">
		...
		<?php
				...
				if (mysqli_connect_errno()){
					...
					$conx = mysqli_connect("localhost", "user", "password", "homhist");
					if (mysqli_connect_errno()){
						echo "Error: ".mysqli_connect_errno();
						$hist=0;
					}else{
					...
					if ( isset($_POST['restart']) ){
						if($hist==1){
							$str="RESTART SYSTEMU-www";
							$today_tim = date("H:i:s");
							$today = date("Y-m-d");
							mysqli_query($conx, 'INSERT INTO syst (dat, tim, opis) VALUES ("'. $today . '", "' . $today_tim .'", "'. $str . '");');
						}
						mysqli_query($con, 'UPDATE cnf SET valu=1 WHERE comm="restart";');
						echo '<div class="dpList">';
						echo '<h2>Odczekaj chwilę na ponowne uruchomienie i kliknij w [IDR] w lewym górnym rogu.</h2>';
						echo '</div>';
						echo '<br />';
					}else{
						echo '<div class="dpList">';
						echo '<h2>Czy na pewno chcesz zrestartować system?</h2>';

						echo '				<form action="" method="POST">';
						echo '					<input type="hidden" name="restart" value="1">';
						echo '					<input type="submit" value="Wykonaj restart">';
						echo '				</form>';
